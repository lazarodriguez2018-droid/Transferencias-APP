...
function renderReceiptOrders(){
  ...
  const reservations=receiptReservationData().filter(r=>(filter!=='linked'||r.linked)&&(filter!=='notify')&&(!query||receiptOrderNormalize([r.numero,r.codigo,r.cliente_nombre,r.cliente_apellido,r.motivo_nombre,...(r.items||[]).flatMap(i=>[i.codigo,i.nombre])].join(' ')).includes(query)));
