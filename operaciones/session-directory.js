(function(root){
  ...
  function mount(config){
    ...
    root.document.querySelectorAll('[data-directory-module]').forEach(el=>{el.setAttribute('aria-current',el.dataset.directoryModule===config.module?'page':'false');});
  ...
  function renderFilters(){
    ...
    $('session-filter-body').innerHTML=`<label>Local<select id="session-location-filter" data-filter="local">${selectOptions(locals,f.local,'Todos mis locales')}</select></label>
      ...
      <label>Usuario o invitado<select data-filter="user">${selectOptions(users,f.user,'Todas las personas')}</select></label><p class="directory-hint">Incluye al creador y a quienes participaron. En invitados, agrupa los ingresos con el mismo nombre.</p>
      ...
      ${stock?`<fieldset><legend>Stock del archivo</legend><label>Disponibilidad<select data-filter="stock">${Object.entries(stocks).map(([v,l])=>`<option value="${v}"${f.stock===v?' selected':''}>${l}</option>`).join('')}</select></label><div class="directory-date-range"><label>Mínimo<input type="number" data-filter="stock_min" step="any" inputmode="decimal" value="${html(f.stock_min)}" placeholder="Sin mínimo"></label><label>Máximo<input type="number" data-filter="stock_max" step="any" inputmode="decimal" value="${html(f.stock_max)}" placeholder="Sin máximo"></label></div><p class="directory-hint">${options.module==='inventario'?'Saldo del balance cargado en ese inventario.':'Stock de origen informado en el archivo de reposición.'} No es stock en tiempo real. Sin archivo o sin dato no equivale a cero.</p></fieldset>`:'<p class="directory-hint">En los remitos se comparan las cantidades esperadas y recibidas; no representan el stock del local.</p>'}
      <p class="directory-hint">Producto, categoría, cantidad y stock deben coincidir en el mismo producto.</p><button type="button" class="btn btn-p btn-full directory-show-results">Ver resultados</button>`;
