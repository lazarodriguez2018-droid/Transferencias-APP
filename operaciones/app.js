...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (window.SucanCloud) {
    try {
      ...
      serverOnline = true;
      const pdata = await fetch('/api/padron').then(response => response.json());
      padron = pdata.padron || [];
...
async function crearSesion() {
  ...
  try {
    const res = await fetch(`${url}/api/sesion/crear`, {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({nombre_sesion: nombreSesion, nombre_usuario: usuario})
    });
    const data = await res.json();
    if (!data.ok) { showSessionError(data.error || 'Error'); return; }
...
async function updateClientCount() {
  ...
  try {
    ...
    if (el) el.textContent = `${data.connected} dispositivo(s) conectado(s)`;
  } catch (e) {}
  if (serverOnline) setTimeout(updateClientCount, 5000);
}
...
function applySavedTheme() {
  ...
  if (button) button.textContent = theme === 'dark' ? 'Modo claro' : 'Modo oscuro';
  const meta = document.querySelector('meta[name="theme-color"]');
  if (meta) meta.content = theme === 'dark' ? '#101a24' : '#f4f7fb';
}
...
async function cambiarSesion() {
  ...
  if (confirmed) {
    if (currentModule === 'reposicion') {
      ...
      serverOnline = false;
      repoState = null;
    } else if (serverOnline && sessionId) {
      fetch(`${serverUrl}/api/sesion/salir`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({session_id: sessionId, nombre_usuario: usuarioNombre})
      }).catch(()=>{});
    }
    if (currentModule !== 'reposicion') disconnectServer();
    if (serverSSE) { serverSSE.close(); serverSSE = null; }
    sessionId = ''; sessionNombre = '';
    document.getElementById('main-nav').style.display = 'none';
    document.getElementById('main-tabs').style.display = 'none';
    document.getElementById('repo-tabs').style.display = 'none';
    mostrarPantallaSesion();
