...
async function joinReposition(rid, nombre, url, usuario, initialRepo) {
  try {
    serverUrl = url;
    serverOnline = true;
    sessionId = rid;
...
function repoFindInitialIndex() {
  ...
  const pending = repoState.items.findIndex(item => !['completo', 'excedido', 'no_encontrado', 'incompleto'].includes(SucaneitorReposition.status(item)));
  ...
  if (window.SucanCloud) {
    ...
    const refresh = async () => {
      ...
      try {
        ...
        if (response.ok && data.ok) { repoState = data.repo; renderRepositionAll(); setSyncStatus('online'); }
      } finally { refreshing = false; }
    };
    const channel = window.SucanCloud.watchReposition(sessionId, refresh);
    repoSSE = { close() { window.SucanCloud.db.removeChannel(channel); } };
    return;
  }
...
function repoCanEdit() { return repoState?.estado === 'preparando'; }
function requireRepoEditable() {
  if (repoCanEdit()) return true;
  toast('Esta reposición ya fue enviada. Solo se puede consultar y actualizar el número de remito.','w');
  return false;
}

function renderRepoCurrent() {
  const card = document.getElementById('repo-current-card');
  if (!card || !repoState || !repoState.items.length) return;
  ...
  const reasonHtml = registeredReason ? `<div class="app-dialog-product" style="margin-top:10px"><strong>${esc(registeredReason + reasonDetail)}</strong>${item.comentario ? `<span style="font-family:'Sora','Segoe UI',sans-serif">${esc(item.comentario)}</span>` : ''}</div>` : '';
  ...
  const controls = repoCanEdit() ? `
    ...
    : '<div class="app-dialog-product" style="margin-top:14px"><strong>Preparación cerrada</strong><span>Esta mercadería ya fue marcada como enviada. El detalle queda en modo consulta.</span></div>';
  card.innerHTML = `
...
function repoFound(encodedCode) {
...
function confirmRepoModalQty() {
  ...
  const total = window._repoQtyMode === 'add' ? Number(item.preparado) + value : value;
  closeModal();
  repoSetAbsolute(repoEncoded(code),total,window._repoQtyMode === 'add' ? 'encontrado' : 'manual');
}
...
async function repoChangeQty(encodedCode, delta, source) {
  ...
  return repoSetAbsolute(encodedCode, Number(item.preparado || 0) + Number(delta || 0), source || 'manual');
}
...
async function repoSetAbsolute(encodedCode, value, source) {
  ...
  try {
    const response = await repoApi('/api/reposicion/update_qty', {
      method:'POST', headers:{'Content-Type':'application/json'},
      body:JSON.stringify({reposition_id:sessionId,codigo:code,absolute:qty,usuario:usuarioNombre,source})
    });
    const data = await response.json().catch(() => ({}));
    ...
    tactileFeedback(24);
    renderRepositionAll();
    if (qty >= Number(item.pedido) && source !== 'lista') repoSelectNext(true);
    return true;
...
function repoSelectNext(preferPending, direction = 1) {
...
function renderRepoSummary() {
  ...
  if (dispatchButton) dispatchButton.style.display = sent ? 'none' : '';
  if (remitoButton) {
    remitoButton.style.display = sent ? '' : 'none';
...
function startRepoUrgentWatcher() {
  ...
  if (!window.SucanCloud || !repoState || repoState.estado !== 'preparando') return;
