...
const tipoCuenta = el('reg-tipo').value;
...
if(tipoCuenta==='local' && !localVal) return showErr('reg-error','Seleccioná el local para cuentas tipo Local.');
...
const [localNombre,almacen]=localVal?localVal.split('|'):['PERSONAL','PRS'];
...
safeSet('sidebar-local-badge', isAdmin && currentPerfil.local_nombre==='PERSONAL' ? 'Supervisor General' : (currentPerfil.local_nombre+' ('+currentPerfil.almacen+')'));
...
const titles={
dashboard:'Dashboard',misPedidos:(currentPerfil&&currentPerfil.role==='admin')?'Pedidos solicitados':'Pedidos de mi local',paraEnviar:'Pedidos a despachar',
...
const isAdmin = currentPerfil.role==='admin';
const local   = currentPerfil.local_nombre;
const titleEl=document.querySelector('#view-misPedidos h1');
if(titleEl) titleEl.textContent=isAdmin?'📤 Pedidos solicitados':'📤 Pedidos de mi local';
...
const isAdmin = currentPerfil.role==='admin';
const local   = currentPerfil.local_nombre;
const subtitle=document.querySelector('#view-paraEnviar .page-header p');
if(subtitle && isAdmin) subtitle.textContent='Pedidos pendientes a realizar de todos los locales.';
...
'<td>'+escHtml(u.local_nombre||'—')+' ('+escHtml(u.almacen||'—')+')</td>'+
...
el('reg-tipo')?.addEventListener('change', ()=>{
const isLocal=el('reg-tipo').value==='local';
const wrap=el('reg-local')?.closest('.form-group');
if(wrap) wrap.style.opacity=isLocal?'1':'0.6';
if(!isLocal) el('reg-local').value='';
});
...
el('perfil-local').value = (p.local_nombre==='PERSONAL'?'Supervisor General':(p.local_nombre+' ('+p.almacen+')'));
// Foto actual
