...
(async()=>{
  ...
  try{
    ...
    assert.match(await page.locator('.product-search-types').innerText(),/Nombre[\s\S]*Código[\s\S]*Barras/,'Reservas debe ofrecer los mismos tres modos de búsqueda que el conteo de inventario');
    await page.locator('#product-search').fill('zee selva correa');
    ...
    await page.locator('[data-item-field="separacion_estado"]').first().selectOption('pendiente');
    assert.match(await page.locator('.item-editor').first().innerText(),/falta retirarlo de góndola o depósito general/i,'Un producto local pendiente debe explicar que todavía no está separado');
    ...
    assert.match(await page.locator('.next-step').innerText(),/Actualizar separaciones y llegadas[\s\S]*Actualizar cantidades/,'Una reserva mixta debe distinguir separaciones locales y llegadas externas');
    await page.locator('[data-next-step="receive"]').click();
    assert.match(await page.locator('#arrival-form').innerText(),/Actualizar separaciones y llegadas[\s\S]*Separadas[\s\S]*Recibidas aquí/i,'La actualización mixta debe rotular cada cantidad según su origen');
    ...
    assert.match(labelLayout.text,/MOTIVO: Pedido web · Agencia o correo[\s\S]*OBSERVACIONES: Retira mañana por la tarde/,'La etiqueta debe explicar por qué se separó y conservar las observaciones iniciales');
    assert.doesNotMatch(labelLayout.text,/SUCAN · CONTROL DE RESERVAS/,'La etiqueta no debe mostrar la firma anterior');
    assert.doesNotMatch(labelLayout.text,/Origen mixto|Esperando proveedor|Pedido a otro local|Ya estaba en local/,'La etiqueta no debe confundir el motivo general con el origen de un producto');
    ...
    const publicPage=await browser.newPage({viewport:{width:390,height:844},isMobile:true,hasTouch:true});
    const publicStub=supabaseStub.replace("if(name==='op_reserva_contexto')return {data:context,error:null};","if(name==='op_reserva_contexto')return {data:args?.p_acceso?{...context,actor:{name:'Acceso de creación',local:'PDE',warehouse:'01',supervisor:false,role:'creador',public_create:true}}:context,error:null};");
    await publicPage.route('**/npm/@supabase/supabase-js@2**',route=>route.fulfill({contentType:'application/javascript',body:publicStub}));
    await publicPage.route('**/npm/qrcodejs@1.0.0/**',route=>route.fulfill({contentType:'application/javascript',body:'window.QRCode=function(){};QRCode.CorrectLevel={M:0};'}));
    await publicPage.route('**/npm/jsbarcode@3.12.3/**',route=>route.fulfill({contentType:'application/javascript',body:'window.JsBarcode=function(){};'}));
    await publicPage.route('https://fonts.googleapis.com/**',route=>route.fulfill({contentType:'text/css',body:''}));
    const createToken='b'.repeat(64);
    await publicPage.goto(`${target}#${createToken}`,{waitUntil:'domcontentloaded'});
    await publicPage.waitForSelector('#app:not([hidden])');
    assert.equal(await publicPage.locator('body').evaluate(body=>body.classList.contains('public-create')),true,'El enlace debe entrar directamente al modo de creación');
    assert.equal(await publicPage.locator('#access-missing').isHidden(),true,'Un enlace válido no debe mostrar una pantalla de acceso');
    assert.equal(await publicPage.locator('.tabs').isHidden(),true,'El enlace no debe mostrar listas, historial ni configuración');
    assert.equal(await publicPage.locator('#view-new').evaluate(section=>section.classList.contains('active')),true,'El formulario debe ser la única vista activa');
    assert.equal(await publicPage.locator('#new-local').isDisabled(),true,'El local debe quedar fijado por el enlace');
    assert.equal(await publicPage.locator('#new-local').inputValue(),'PDE','El enlace debe conservar el local configurado');
    assert.equal(await publicPage.locator('#new-responsible').inputValue(),'','El enlace no debe pedir identidad antes de abrir; el responsable se completa en la reserva');
    assert.equal(await publicPage.locator('#manage-link').innerText(),'Gestionar reservas','La gestión debe dirigir al inicio de sesión');
    await publicPage.locator('#new-responsible').fill('Empleado del enlace');
    await publicPage.locator('#customer-name').fill('Cliente enlace');
    await publicPage.locator('#product-search').fill('CORREA ZEE');
    await publicPage.waitForSelector('#product-results [data-product-index="0"]');
    await publicPage.locator('#product-results [data-product-index="0"]').click();
    assert.equal(await publicPage.locator('[data-item-field="separacion_estado"]').inputValue(),'','La situación física no debe presumirse por estar disponible en el local');
    await publicPage.locator('[data-item-field="separacion_estado"]').selectOption('completa');
    assert.match(await publicPage.locator('.item-editor').innerText(),/Ya está separado para esta reserva[\s\S]*próximo paso será avisar, despachar o entregar/i,'La creación debe reconocer la mercadería ya separada sin pedir una llegada posterior');
    ...
    assert.equal(publicCalls.find(call=>call.name==='op_reserva_crear_v2').args.p_datos.items[0].cantidad_local,1,'La opción ya separada debe registrar toda la cantidad como físicamente disponible');
    ...
    const lookupStub=`window.supabase={createClient:()=>({auth:{getSession:async()=>({data:{session:null}})},rpc:async()=>({data:{ok:true,reservation:{number:1,code:'SUCAN001',local:'PDE',warehouse:'01',reason:'Pedido web',purpose:'pedido_web',delivery:'agencia',observations:'Retira mañana por la tarde',customer:'Ana Suárez',phone:'099 123 456',responsible:'Empleado prueba',state:'listo',created_at:new Date().toISOString(),merchandise_at:new Date().toISOString(),expires_at:new Date(Date.now()+86400000).toISOString(),reference:'WEB-1001',location:'Estante de pruebas',items:[{codigo:'010031110010408',nombre:'CORREA ZEE DOG - SELVA - XS',cantidad:3,cantidad_local:3,cantidad_entregada:0,procedencia:'local'}]}},error:null})})};`;
    ...
    assert.match(await lookupPage.locator('.reservation-summary').innerText(),/Motivo y observaciones generales[\s\S]*Pedido web · Agencia o correo[\s\S]*Retira mañana por la tarde/,'Al escanear el QR deben verse claramente el motivo y las observaciones generales');
    ...
    assert.match(await lookupPopup.locator('.label').innerText(),/MOTIVO: Pedido web · Agencia o correo[\s\S]*OBSERVACIONES: Retira mañana por la tarde/,'La reimpresión pública debe conservar motivo y observaciones');
    assert.doesNotMatch(await lookupPopup.locator('.label').innerText(),/SUCAN · CONTROL DE RESERVAS/,'La reimpresión pública no debe mostrar la firma anterior');
    assert.doesNotMatch(await lookupPopup.locator('.label').innerText(),/Origen mixto/,'La reimpresión pública no debe confundir el motivo con el origen de los productos');
