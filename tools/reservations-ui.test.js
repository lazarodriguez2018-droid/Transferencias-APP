...
assert.match(js,/Cantidad reservada[\s\S]*Cantidad ya separada/,'La creación debe distinguir cantidad reservada y cantidad ya separada');
assert.match(js,/Unidades separadas aquí[\s\S]*Solo las que están físicamente juntas para esta reserva[\s\S]*Solicitadas[\s\S]*Ya entregadas/,'El detalle debe explicar las cantidades operativas');
...
assert.match(page,/reservas\.css\?v=6[\s\S]*reserva-engine\.js\?v=2[\s\S]*reservas\.js\?v=11/,'El enlace posterior a la creación debe invalidar la caché anterior');
assert.match(lookupPage,/consulta\.css\?v=2[\s\S]*qrcodejs@1\.0\.0[\s\S]*reserva-engine\.js\?v=2[\s\S]*consulta\.js\?v=5/,'La reimpresión pública debe cargar el generador QR e invalidar la caché');
