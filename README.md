...
`/reservas` registra una reserva completa por cliente o destinatario, con varios productos, responsable, motivo, procedencia y cantidades. El buscador de productos reutiliza los modos Nombre, Código y Barras y el motor inteligente del conteo de Control de inventario. Puede crear automáticamente un pedido entre locales o vincular uno ya existente. El ingreso de mercadería también se integra con el control de remitos. Cada tienda puede configurar el lugar fijo donde guarda las reservas. Una etiqueta térmica de 80 mm incluye nombre, motivo por el que se separó, observaciones iniciales, fecha, ubicación y un QR interno de consulta; al escanearlo se muestran claramente el motivo y las observaciones generales. Modificar datos exige autenticación o el enlace rápido privado del local más la contraseña de empresa.
