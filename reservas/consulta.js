(async function(){
  ...
  const itemOrigin=i=>i.procedencia==='proveedor'?`Esperando proveedor${i.proveedor_nombre?' · '+i.proveedor_nombre:''}`:i.procedencia==='pedido_local'?`Pedido a otro local${i.origen_local?' · Desde '+i.origen_local:''}${i.pedido_local_gestion==='crear'?' · Pedido creado':i.pedido_local_gestion==='existente'?' · Pedido vinculado':i.pedido_local_gestion==='externo'?' · Coordinado fuera del sistema':''}`:(E.sources[i.procedencia]||i.procedencia);
  ...
  try{
    ...
    const r=data.reservation,d=E.deadlineInfo(r.expires_at),number=r.number??r.code;$('lookup-code').textContent=`RESERVA #${number} · ${r.local}`;$('lookup-customer').textContent=r.customer||'Sin cliente';$('lookup-reason').textContent=`${r.reason} · Responsable: ${r.responsible}`;$('lookup-state').textContent=E.labels[r.state]||r.state;$('lookup-state').className=`status ${r.state}`;
    ...
    const {data:{session}}=await db.auth.getSession();$('lookup-manage').href=session?`/reservas?qr=${encodeURIComponent(token)}`:'/?module=reservas';$('lookup-manage').textContent=session?'Abrir reserva':'Iniciar sesión';$('lookup-loading').hidden=true;$('lookup-content').hidden=false;
