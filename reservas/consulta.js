(async function(){
  ...
  try{
    ...
    $('lookup-data').innerHTML=`<div class="row"><span>Teléfono</span><strong>${html(r.phone||'—')}</strong></div><div class="row"><span>Creada</span><strong>${fmt(r.created_at)}</strong></div><div class="row"><span>Mercadería desde</span><strong>${fmt(r.merchandise_at)}</strong></div><div class="row"><span>Plazo</span><strong>${html(d.label)}</strong></div><div class="row"><span>Referencia</span><strong>${html(r.reference||'—')}</strong></div>`;
    $('lookup-products').innerHTML=(r.items||[]).map(i=>`<div class="product"><strong>${html(i.nombre)} · ${i.cantidad}</strong><small>${html(i.codigo)} · ${html(E.sources[i.procedencia]||i.procedencia)} · ${i.cantidad_local} en local · ${i.cantidad_entregada} entregadas</small></div>`).join('')||'<p>Sin productos.</p>';
